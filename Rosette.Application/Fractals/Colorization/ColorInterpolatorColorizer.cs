...
{
    /// <summary>Class able to take an arbitrary number of colors, and smoothly loop through them.</summary>
    class ColorInterpolatorColorizer : IColorListColorizer
    {
        public Color InnerColor { get; set; }
        public IList<Color> Colors { get; set; }

        /// <summary>Creates a new ColorInterpolatorColorizer with an arbirary number of colors to use.</summary>
        public ColorInterpolatorColorizer()
        {
        ...
        public Color GetColor(float value, bool IsMaxIterations)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                colorBytes[i * 3] = Colors[(increment + i) % Colors.Count].R;
                colorBytes[i * 3 + 1] = Colors[(increment + i) % Colors.Count].G;
                colorBytes[i * 3 + 2] = Colors[(increment + i) % Colors.Count].B; 
            }
